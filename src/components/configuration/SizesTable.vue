<template>
  <div>
    <DataTable title="Sizes" :columns="columns" :data="configStore.sizes" :loading="configStore.loading"
      :error="configStore.error" :get-row-id="(row) => row.id" @create="showCreateModal = true" @edit="handleEdit"
      @delete="handleDelete" />
    <Modal :is-open="showModal" :title="editingSize ? 'Edit Size' : 'Create Size'" :show-footer="true"
      @close="closeModal">
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Value</label>
          <input v-model.number="formData.value" type="number" required step="0.01"
